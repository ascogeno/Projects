...
remainder(Dividend, 0) ->
    fail;
remainder(Dividend, Divisor) when is_integer(Dividend), is_integer(Divisor) ->
...
divide_test_() ->
    [% happy path, tests the obvious case. If we divide 10 and 5 we want it to
     ...
     ?_assertEqual(fail,
                   divide(3, 0)), % This should return the atom fail if we try to divide by 0.
