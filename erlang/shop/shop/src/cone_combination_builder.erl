...
cone_combinations(Top_flavors, Bottom_flavors)
    when is_list(Top_flavors), is_list(Bottom_flavors) ->
...
cone_combinations(nil, _) ->
    fail;
cone_combinations(_, nil) ->
    fail;
cone_combinations(_, _) ->
    fail.%I tried writing this to have fewer lines of code, but eunit just wasn't budging.
         % If there's a way to get this to pass with fewer edge case checks, I'm all ears.

% TODO: Complete this function so it passes all of its unit tests
cone_combinations(Flavor_list) when is_list(Flavor_list) ->
    [{X, Y} || X <- Flavor_list, Y <- Flavor_list];
cone_combinations(_) ->
    fail.

% TODO: Complete this function so it passes all of its unit tests
most_popular_combinations(Count, List) when Count >= 0 ->
...
cone_combinations_two_list_test_() ->
    ...
    [?_assertEqual(Short_combinations,
                   cone_combinations(Top_scoop_list, Bottom_scoop_list)), % happy path
...
cone_combinations_one_list_test_() ->
    ...
    All_combinations =
        [{vanilla, vanilla}, {vanilla, chocolate}, {vanilla, cherry_ripple}, {vanilla, lemon},
         {vanilla, butterscotch}, {vanilla, licorice_riple}, {chocolate, vanilla},
         {chocolate, chocolate}, {chocolate, cherry_ripple}, {chocolate, lemon},
         {chocolate, butterscotch}, {chocolate, licorice_riple}, {cherry_ripple, vanilla},
         {cherry_ripple, chocolate}, {cherry_ripple, cherry_ripple}, {cherry_ripple, lemon},
         {cherry_ripple, butterscotch}, {cherry_ripple, licorice_riple}, {lemon, vanilla},
         {lemon, chocolate}, {lemon, cherry_ripple}, {lemon, lemon}, {lemon, butterscotch},
         {lemon, licorice_riple}, {butterscotch, vanilla}, {butterscotch, chocolate},
         {butterscotch, cherry_ripple}, {butterscotch, lemon}, {butterscotch, butterscotch},
         {butterscotch, licorice_riple}, {licorice_riple, vanilla}, {licorice_riple, chocolate},
         {licorice_riple, cherry_ripple}, {licorice_riple, lemon}, {licorice_riple, butterscotch},
         {licorice_riple, licorice_riple}],
    % Do the actual tests
    [?_assertEqual(All_combinations,
                   cone_combinations(Top_scoop_list ++ Bottom_scoop_list)),%happy path
     % Less obvious things we need to test start start here, the edge cases.
     ?_assertEqual([], cone_combinations([])),
     ?_assertEqual(fail, cone_combinations(nil))].
...
most_popular_combinations_test_() ->
    ...
    Sorted_combinations_with_purchases =
        lists:sort(
            lists:zip([rand:uniform(100000) || _ <- lists:seq(0, length(All_combinations) - 1)],
                      All_combinations)),
    ...
    [?_assertEqual(H,
                   most_popular_combinations(10, Sorted_combinations_with_purchases)),% Happy path
     ...
     ?_assertEqual(fail,
                   most_popular_combinations(5,
                                             [{86656, {vanilla, vanilla}},
