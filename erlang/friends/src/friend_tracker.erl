...
start(Initial_friends_list) ->
    spawn(?MODULE,
          run,
          [Initial_friends_list]). % The MODULE macro is used instead of hard coding the module name.
    ...
    Updated_list =
        receive
            ...
            {Pid, {has_friends, Friends}} when is_list(Friends) ->
                Pid ! lists:all(fun(Friend) -> lists:member(Friend, Friend_list) end, Friends),
                Friend_list;
            {Pid, {has_friends, []}} ->
                Pid ! lists:member([], Friend_list),
                Friend_list;
            {Pid, {remove, Friend}} ->
                Pid ! received,
                Friend_list -- [Friend];
...
add_friend_test_() ->
    {setup,
     fun() ->
        % runs before any of the tests to set up the test
     ...
     [?_assertEqual(received, rpc(test_adder, {add, bob})), % Test obvious case
      ...
      ?_assertEqual(received,
                    rpc(test_adder,
                        {add,
                         #{name => suzannah,
                           age => 23}}))]}. % Yes we can add a map/dictionary to the list
...
has_friend_test_() ->
    {setup,
     ...
     [?_assert(rpc(test_finder, {has_friend, sue})),%happy path. Should return true
      ...
      ?_assertNot(rpc(test_finder,
                      {has_friend,
                       [#{name => suzannah, age => 23}, #{name => gunhild, age => 20}]}))]}.
